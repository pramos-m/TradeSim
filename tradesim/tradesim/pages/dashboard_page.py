...
# Tarjeta Stock Portfolio (igual)
def portfolio_stock_card(item: PortfolioItem) -> rx.Component:
    logo_src = item.logo_url; details_link = rx.link("Ver Detalles", href="#", size="1", color_scheme="blue");
    return rx.card(rx.hstack(rx.image(src=logo_src, width="40px", height="40px", object_fit="contain", fallback="/default_logo.png", mr="3"), rx.vstack(rx.text(item.name, font_weight="medium", size="2", no_of_lines=1), rx.text(f"{item.quantity} Acciones", font_size="1", color=TEXT_GRAY), align_items="start", spacing="0"), rx.spacer(), rx.vstack(rx.text(f"$ {item.current_value:,.2f}", font_weight="medium", size="2", text_align="right"), details_link, align_items="end", spacing="1"), spacing="4", align="center", width="100%"), size="1", bg="white", border_radius="xl", box_shadow="0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.1)", p="4")

# Componente para Fila de Transacción
def transaction_row(transaction: TransactionDisplayItem) -> rx.Component:
    row_color = rx.cond(transaction.type == "Compra", "green.500", "red.500")
    quantity_str = f"{transaction.quantity:+}"
def transaction_row(transaction: TransactionDisplayItem) -> rx.Component:
    row_color = rx.cond(transaction.type == "Compra", "var(--green-9)", "var(--red-9)") # Using Radix colors for consistency
    # No longer need quantity_str here, will use transaction.formatted_quantity directly
    return rx.hstack(
        ...
        rx.text(
            transaction.quantity, # Use the computed var here
            size="2", 
            text_align="right", 
            width="60px", 
            color=row_color
...
def dashboard_page() -> rx.Component:
    ...
    page_content = rx.vstack(
        ...
        rx.vstack(
            ...
            # PNL / Balance Grid (PNLs basados en AAPL, sin gráfico interno)
            rx.grid(
            ...
            rx.card(
                rx.vstack(
                    ...
                    period_selector(AuthState.selected_period, AuthState.set_period), # Llama a actualizar gráfico portfolio
                    rx.plotly(data=AuthState.main_portfolio_chart_figure, config={"displayModeBar": False, "scrollZoom": False}, on_hover=AuthState.portfolio_chart_handle_hover, on_unhover=AuthState.portfolio_chart_handle_unhover, width="100%", height="300px"), # Usa figura portfolio
                    spacing="3", align="stretch", w="100%",
                ), w="100%", size="2", mb="24px"
            ),
            # --- FIN Gráfico Principal ---
            # Sección Mi Portfolio (GRID DE TARJETAS)
            rx.vstack(
                ...
                rx.cond(AuthState.portfolio_items.length() == 0, rx.text("Tu portfolio está vacío.", color=TEXT_GRAY, mt="4")),
                align_items="stretch", width="100%", mb="24px"
            ),
            # --- Transacciones Recientes (CON DATOS) ---
            rx.vstack(
                rx.heading("Transacciones Recientes", size="5", mb="1rem", align_self="flex-start"),
                rx.card( # Contenedor para la lista de transacciones
                    rx.vstack(
                        # Encabezados
                        ...
                        rx.cond(
                            ...
                            rx.center(rx.text("No hay transacciones recientes.", color=TEXT_GRAY, padding_y="4"), width="100%") # Mensaje si vacío
                        ),
                        align_items="stretch", width="100%",
                    ),
                    width="100%", size="1", # Tamaño card más pequeño
                    max_height="300px", # Altura máxima con scroll
                    overflow_y="auto"
                ),
                align_items="stretch", width="100%",
            ),
        ...
        # *** on_mount AÑADIDO AQUÍ al Vstack principal ***
